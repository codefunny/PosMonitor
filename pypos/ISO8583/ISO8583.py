...
class ISO8583:
	...
	_BITS_VALUE_TYPE[60] = ['60','Reserved for national use','LLL',20,'n']
	_BITS_VALUE_TYPE[61] = ['61','Reserved for national use','LLL',29,'n']
	_BITS_VALUE_TYPE[62] = ['62','Reserved for private use','LLL',84,'b']
	_BITS_VALUE_TYPE[63] = ['63','Reserved for private use','LLL',3,'ans']
	...
	def setTransationType(self,type):
		...
		self.MESSAGE_TYPE_INDICATION =binascii.a2b_hex("%s%s" % (typeT,type))
	...
	def __inicializeBitmapValues(self):
		...
		if len(self.BITMAP_VALUES) == 128:
			for cont in range(0,128):
				...
				self.ASCII_VALUES[cont] = self._BIT_DEFAULT_VALUE
		else:
			for cont in range(0,128):
				self.BITMAP_VALUES.append(self._BIT_DEFAULT_VALUE)
				self.ASCII_VALUES.append(self._BIT_DEFAULT_VALUE)
	################################################################################################		
	...
	def __setBitTypeLL(self, bit, value):
		...
		size ="%s"% len(value)	
		if self.getBitValueType(bit) == 'n':
			vlen = len(value)%2
			value = value.ljust(len(value)+vlen,'0')
			value = "%s"%binascii.a2b_hex(value)
			
		self.BITMAP_VALUES[bit] = "%s%s" %(binascii.a2b_hex(size.zfill(2)), value)
	...
	def __setBitTypeLLL(self, bit, value):
		...
		if self.getBitValueType(bit) == 'n':
			...
			value = value.ljust(len(value)+vlen,'0')
			value = "%s"% binascii.a2b_hex(value)
		elif self.getBitValueType(bit) == 'b':
			vlen = len(value)%2
			value = value.ljust(len(value)+vlen,'0')
			value = "%s"% binascii.a2b_hex(value)
			size = "%s"%len(value)
		
		self.BITMAP_VALUES[bit] = "%s%s" %(binascii.a2b_hex( size.zfill(4)),value)
	...
	def __setBitTypeN(self, bit, value):
		...
		if len(value) > self.getBitLimit(bit):
			...
			raise ValueToLarge('Error: value up to size! Bit[%s] of type %s limit size = %s' % (bit,self.getBitType(bit),self.getBitLimit(bit)) )
		value = value.ljust(self.getBitLimit(bit)+self.getBitLimit(bit)%2,'0')
		if self.getBitValueType(bit) == 'n':
	...
	def getRawIso(self):
		...
		if self.BITMAP_VALUES[64] <> self._BIT_DEFAULT_VALUE:
			mblock = copy.deepcopy(resp[11:])
			calmac = self.getMac(mblock)
			self.BITMAP_VALUES[64] = "%s"%calmac
			#print "%s"%calmac
			resp = "%s%s"%(resp,self.BITMAP_VALUES[64])
		#print "resp = [%s]"%resp
		return resp
	...
	###############################################################################################
	def getMac(self,macblock):
		f = file("tmk.etc",'r')
                mackey = f.readline().strip()
                f.close()
                mk1 = mackey[:]

                xdata = list(binascii.a2b_hex("0000000000000000"))
                macb = macblock[:]
		
                #print "mabl = %s"%binascii.b2a_hex(macb)
                mlen = len(macb)

                for j in range(0,8):
                        tmp1 = binascii.b2a_hex(xdata[j])
                        if j < mlen:
                                tmp2 = binascii.b2a_hex(macb[j])
                                tmp3 = "%x"%(int(tmp1,16) ^ int(tmp2,16))
                        else:
                                tmp3 = "%x"%(int(tmp1,16) ^ 0x00)
                        tmp3 = "%s"%tmp3
                        tmp3 = tmp3.zfill(2)
                        tmp4 = binascii.a2b_hex(tmp3)
                        xdata[j] ="%s"% tmp4
                i = 8
                k1 = pyDes.des(binascii.a2b_hex(mk1),IV='\0\0\0\0\0\0\0\0')
                while i < mlen:
                        for j in range(0,8):
                                tmp1 = binascii.b2a_hex(xdata[j])
                                if i+j < mlen:
                                        tmp2 = binascii.b2a_hex(macb[i+j])
                                        tmp3 = "%x"%(int(tmp1,16) ^ int(tmp2,16))
                                else:
                                        tmp3 = "%x"%(int(tmp1,16) ^ 0x00)
                                tmp3 = "%s"%tmp3
                                ...
                                tmp4 = binascii.a2b_hex(tmp3)
                                xdata[j] ="%s"% tmp4
			#print "xor mb %d,%s"%(i,binascii.b2a_hex("".join(xdata)))
                        i += 8
                xdata1 = binascii.b2a_hex("".join(xdata)).upper()
                #print "xdata1 = [%s]"%xdata1
		xdata2 = xdata1[:8]
		xdata3 = xdata1[8:]
		#print "2 :%s,3: %s"%(binascii.b2a_hex(xdata2),binascii.b2a_hex(xdata3))
                ek = k1.encrypt(xdata2)

		xorout = self.xorstr(ek,xdata3)
		ek1 = k1.encrypt(xorout)

                mac =binascii.b2a_hex(ek1)[:8].upper()
		
                print "mac is %s"%mac
                return mac
	################################################################################################
	def xorstr(self,str1,str2):
		mlen = len(str2)
		xdata = list(str1)
		...
		for j in range(0,8):                                                               
                	tmp1 = binascii.b2a_hex(xdata[j])                                          
                        ...
                        xdata[j] ="%s"% tmp4                                                       
		if self.DEBUG == True:
                	print "xor1 mb %s"%(binascii.b2a_hex("".join(xdata)))                          
		...
		for cont in range(2,128):
			if self.BITMAP_VALUES[cont] <> self._BIT_DEFAULT_VALUE:
				...
				if self.getBitType(cont) == 'LL':
					...
					tmpValue = strWithoutMtiBitmap[offset+1:offset+1+valueSize]
					if self.getBitValueType(cont) == 'n':
						valueSize = valueSize/2 + valueSize%2
						tmpValue = strWithoutMtiBitmap[offset+1:offset+1+valueSize]
						tmpValue = binascii.b2a_hex(tmpValue)
					self.BITMAP_VALUES[cont] = tmpValue
				...
				if self.getBitType(cont) == 'LLL':
					valueSize = int(binascii.b2a_hex(strWithoutMtiBitmap[offset:offset +2]))
					if self.DEBUG == True:
					...
					if self.getBitValueType(cont) == 'n' :
						oriValueSize = valueSize
						valueSize = valueSize/2 + valueSize%2
						tmpValue = binascii.b2a_hex(strWithoutMtiBitmap[offset+2:offset+2+valueSize])
						tmpValue = tmpValue[:oriValueSize]
					elif  self.getBitValueType(cont) == 'b' :
						tmpValue = binascii.b2a_hex(strWithoutMtiBitmap[offset+2:offset+2+valueSize])	
					else:
						tmpValue = strWithoutMtiBitmap[offset+2:offset+2+valueSize]
				...
				if self.getBitType(cont) == 'N':
					end = self.getBitLimit(cont)	
					tmpValue = strWithoutMtiBitmap[offset:offset+end]
 					if self.getBitValueType(cont) == 'n':
						end = self.getBitLimit(cont)/2+self.getBitLimit(cont)%2
						tmpValue = binascii.b2a_hex(strWithoutMtiBitmap[offset:offset+end])[:
self.getBitLimit(cont)]			
					self.BITMAP_VALUES[cont] = tmpValue
					if self.DEBUG == True:	
						print '\tSetting bit N %s value %s' % (cont,self.BITMAP_VALUES[cont])
					offset += end	
					
				if self.getBitType(cont) == 'B':
 					if self.getBitValueType(cont) == 'b':
						end = self.getBitLimit(cont)/2+self.getBitLimit(cont)%2
						tmpValue = binascii.b2a_hex(strWithoutMtiBitmap[offset:offset+end])[:
self.getBitLimit(cont)]
					else:
					...
					self.BITMAP_VALUES[cont] = tmpValue
					if self.DEBUG == True:	
						print '\tSetting bit B %s value %s' % (cont,self.BITMAP_VALUES[cont])
					offset += end		
